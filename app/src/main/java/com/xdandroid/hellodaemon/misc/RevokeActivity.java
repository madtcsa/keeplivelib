...
import com.xdandroid.hellodaemon.task.ThreadPool;
                ...
                try {
                    ...
                    for (PackageInfo pi : installedPackages) {
                        ...
                        for (String reqPermission : pi.requestedPermissions) {
                            ...
                            if (permissionInfo == null || permissionInfo.protectionLevel != PermissionInfo.PROTECTION_DANGEROUS) {
                                continue;
                            }
                            if (!permissionInfo.name.startsWith("android")) {
                                continue;
                            }
                            if (nougat) {
                                String op = AppOpsManager.permissionToOp(permissionInfo.name);
                                if (!TextUtils.isEmpty(op)) {
                                    setUidModeMethod.invoke(aom, op, uid, AppOpsManager.MODE_IGNORED);
                                }
                            } else {
                                try {
